...
// loading literal val 'kk' into register whatever is already in 'Vx' (LD)
void Chip8::OP_6xkk()
{
	uint8_t Vx = (opcode & 0x0F00u) >> 8u;
	uint8_t byte = opcode & 0x00FFu;

	// load byte data into register
	registers[Vx] = byte;
}

// adding the literal val 'kk' into whatever is already in register 'Vx' (ADD)
void Chip8::OP_7xkk()
{
	uint8_t Vx = (opcode & 0x0F00u) >> 8u;
	uint8_t byte = opcode & 0x00FFu;
...
// key press (wait) then store
void Chip8::OP_Fx0A()
{
	// this function DOESNT know which key it cares about, so we check everything
	uint8_t Vx = (opcode & 0x0F00u) >> 8u;
	...
	{
		// here we de-increment and Cycle() will increment (so cancelling one another)
		// this will then continue until a key is pressed
		pc -= 2;
	}
}

// setting the delay timer to Vx
void Chip8::OP_Fx15()
{
	uint8_t Vx = (opcode & 0x0F00u) >> 8u;
